...
const TrendIcon = ({ trend }: { trend: string }) => {
  if (trend === 'up') return <span className="text-[#16a34a] text-xs">↑</span>;
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="overflow-x-auto rounded-2xl border border-gray-100 shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-[#f0fdf4] text-gray-600 uppercase text-xs tracking-wider">
            <tr>
          ...
          <tbody>
            {data.map((row, i) => (
              <tr key={row.team} className={`border-t border-gray-100 ${i === 0 ? 'bg-green-50/50' : 'hover:bg-gray-50'} transition-colors`}>
                <td className="px-4 py-3 font-semibold text-gray-700">
                  <div className="flex items-center gap-1.5">
                    {row.pos === 1 && <span className="text-[#16a34a]">🏆</span>}
                    {row.pos}
                ...
                <td className="px-4 py-3 text-center font-bold text-base">{row.pts}</td>
      ...
      <p className="text-xs text-gray-400 mt-4">* Table updated after each matchday. Last updated: 3 May 2026</p>
    </div>
  );
}
